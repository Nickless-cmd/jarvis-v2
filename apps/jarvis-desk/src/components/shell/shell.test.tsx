...
describe('shell', () => {
  ...
  it('Composer sends on Enter with opts, not Shift+Enter', async () => {
    const onSend = vi.fn()
    render(<Composer streaming={false} onSend={onSend} onStop={() => {}} model="deepseek-flash" thinking="think" />)
  ...
  it('Composer keeps text on Shift+Enter (no send)', async () => {
    const onSend = vi.fn()
    render(<Composer streaming={false} onSend={onSend} onStop={() => {}} model="m" thinking="think" />)
    const ta = screen.getByRole('textbox')
  ...
  it('Composer viser stop-knap der kalder onStop under streaming', async () => {
    const onStop = vi.fn()
    render(<Composer streaming onSend={() => {}} onStop={onStop} model="m" thinking="think" />)
