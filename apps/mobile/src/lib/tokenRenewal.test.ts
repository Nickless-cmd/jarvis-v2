import { laesKrav, boerFornys, fornyOmNoedvendigt } from './tokenRenewal'
import type { ApiConfig } from './types'

/**
 * Mikkels telefon gav 927 × 401 på seks timer med `token expired` og havde
 * ingen vej tilbage. Det disse tests holder på, er de tre ting der gør
...
describe('selve fornyelsen', () => {
  ...
  it('beholder det gamle token når serveren siger nej', async () => {
    ...
    expect((await fornyOmNoedvendigt(cfg, jest.fn(), NU)).authToken).toBe(cfg.authToken)
  })
  ...
  it('vælter ikke opstarten når der ikke er net', async () => {
    ...
    ;(global as any).fetch = jest.fn().mockRejectedValue(new Error('ingen net'))
    const gem = jest.fn()
  ...
  it('gemmer ikke et tomt token', async () => {
    ;(global as any).fetch = jest.fn().mockResolvedValue({ ok: true, json: async () => ({ token: '' }) })
    const gem = jest.fn()
