...
function blocksToAssistantText(blocks: ContentBlock[]): string {
  ...
  // efterlader `undefined`-huller). Rå `.map` ville ramme hullet og crashe på
  // `block.type`. `block &&` er defense-in-depth oveni.
  return denseBlocks(blocks)
    .map((block) => {
      if (block && block.type === 'text') return block.text
      if (block && block.type === 'thinking') return block.thinking
      return ''
    })
    .join('')
}
