...
plus the session-distillation, web-cache, session-topic and daemon-output-log
tables. All `_ensure_*_table` helpers are invoked lazily by their own CRUD
(never by init_db), so the cluster is self-contained.
