...
def produce_signals_from_run(
    *,
    ...
    try:
        from core.services.conversation_rhythm import track_conversation_rhythm
        was_corrected = any(m in msg_lower for m in (
            "nej", "forkert", "ikke det", "stadig samme", "prøv igen", "hold nu"
        ))
    ...
    try:
        from core.services.relationship_texture import update_relationship_from_run
...
def produce_emergent_signals_from_history() -> dict[str, int]:
    ...
    try:
        from core.services.emergent_signal_tracking import run_emergent_signal_daemon
...
def detect_decision_in_message(
    *, user_message: str, assistant_response: str, run_id: str,
    ...
    if not any(m in msg_lower for m in decision_markers):
        return None
    try:
        from core.services.decision_log import record_decision
...
def progress_signal_lifecycles() -> dict[str, int]:
    ...
    try:
        from core.services.reflective_critic_tracking import refresh_runtime_reflective_critic_statuses
    ...
    try:
        from core.services.self_model_signal_tracking import refresh_runtime_self_model_signal_statuses
