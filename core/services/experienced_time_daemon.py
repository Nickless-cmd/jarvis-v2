...
def build_experienced_time_daemon_surface() -> dict[str, object]:
    ...
    return {
        "active": True,
        "mode": "experienced_time_daemon",
        "summary": "Module loaded; entry points available.",
        "authority": "derived-read-only",
    }


def _emit_experienced_time_daemon_event(kind: str, payload: dict[str, object] | None = None) -> None:
    ...
    try:
        ...
        event_bus.publish(
            f"experienced_time_daemon.{kind}",
            payload or {},
        )
    except Exception:
        pass
