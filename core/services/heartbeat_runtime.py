...
from urllib import error as urllib_error  # noqa: F401  (re-exported surface)
from urllib import request as urllib_request  # noqa: F401  (re-exported surface)
...
# Boy-Scout extraction: pure leaf helpers and provider-executor bodies live in
# dedicated submodules. Re-import them into this namespace so existing imports
# (`from core.services.heartbeat_runtime import _estimate_tokens`, ...) and test
# monkeypatch seams (`monkeypatch.setattr(heartbeat_runtime, "_estimate_tokens",
# ...)`) keep working unchanged. Callers inside this module reference these by
# bare name (resolved here); provider bodies resolve their helper siblings back
# through this facade module at call time so patches remain visible.
from core.services.heartbeat_runtime_helpers import (  # noqa: E402
...
from core.services.heartbeat_runtime_providers import (  # noqa: E402
    _execute_groq_prompt,
    _execute_ollama_prompt,
    _execute_openai_prompt,
    _execute_openrouter_prompt,
)
from core.services.heartbeat_runtime_influence import (  # noqa: E402
    _build_influence_trace,
)
...
# _build_influence_trace moved to heartbeat_runtime_influence.py (Boy-Scout
# extraction, 1331 lines). Re-exported at module top; its single caller
# `_build_heartbeat_context` resolves it by bare name in this namespace, and
# test patches on `heartbeat_runtime._build_influence_trace` still apply.
...
# Provider-executor bodies moved to heartbeat_runtime_providers.py (Boy-Scout
# extraction). Re-exported at module top so `heartbeat_runtime._execute_*_prompt`
# imports and monkeypatch seams keep working. See imports near the top of file.
...
def _execute_heartbeat_internal_action(
    *,
    ...
    if action_type == "manage_runtime_work":
        # Udskilt til sin egen fil 15/9-2026 (Boy Scout) — se modulet.
        from core.services.heartbeat_manage_runtime_work import execute_manage_runtime_work

        return execute_manage_runtime_work(tick_id=tick_id, workspace_dir=workspace_dir)
    # --- Cognitive architecture idle actions ---
