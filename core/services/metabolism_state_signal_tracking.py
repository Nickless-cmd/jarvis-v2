"""Metabolism-state signal tracking — migrated onto signal_tracking_framework.
...
the metabolism-specific candidate derivation and the lifecycle-projection surface
enrichment stay here — that is the part unique to this signal.

This is a ``_for_domain`` S-family variant with a bounded metabolism projection
(``metabolism_state`` / ``metabolism_direction`` / ``metabolism_weight`` … plus the
``authority`` / ``layer_role`` / ``canonical_delete_state`` / ``self_erasure_state``
control keys) on the read surface. Those are expressed via ``item_view_fn`` +
``surface_extra_fn``; the surface omits ``recent_history`` (as the original did).
"""
...
def _metabolism_surface_extra(
    ...
    return {
        ...
        "self_erasure_state": "not-self-erasure",
        "summary_extra": {
            "current_state": str(current.get("metabolism_state") or "none"),
            "current_direction": str(current.get("metabolism_direction") or "none"),
            "current_weight": str(current.get("metabolism_weight") or "low"),
            "current_confidence": str(current.get("metabolism_confidence") or "low"),
            "authority": "non-authoritative",
            "layer_role": "runtime-support",
            "canonical_delete_state": "not-canonical-deletion",
            "self_erasure_state": "not-self-erasure",
...
_SPEC = SignalTrackingSpec(
    name="metabolism-state",
    slug="metabolism-state",
    signal_id_prefix="metabolism-state-signal",
    event_prefix="metabolism_state_signal",
    default_signal_type="metabolism-state",
    list_fn=list_runtime_metabolism_state_signals,
    upsert_fn=upsert_runtime_metabolism_state_signal,
    update_status_fn=update_runtime_metabolism_state_signal_status,
    supersede_fn=supersede_runtime_metabolism_state_signals_for_domain,
    supersede_group_field="domain_key",
    supersede_group_kw="domain_key",
    extract_fn=lambda spec, ctx: _extract_metabolism_state_candidates(run_id=str(ctx.get("run_id") or "")),
    stale_after_days=_STALE_AFTER_DAYS,
    refresh_scan_limit=40,
    refreshable_statuses=frozenset({"active", "softening"}),
    stale_status_reason="Marked stale after bounded metabolism-state inactivity window.",
    surface_status_order=("active", "softening", "stale", "superseded"),
    surface_active_statuses=frozenset({"active", "softening"}),
    empty_current_label="No active metabolism support",
    item_view_fn=_with_surface_view,
    surface_extra_fn=_metabolism_surface_extra,
