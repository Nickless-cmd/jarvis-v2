...
def _build_visible_chat_prompt_assembly_impl(
    *,
    ...
    # Fejlede autonome kørsler (2026-09-02). En udbyders fejlbesked gemmes ikke
    # længere som hans svar — men så ville turen fejle USYNLIGT, og netop dét
    # mønster lod runtime kaste hans beslutninger væk i månedsvis. Han skal
    # kunne SE at det skete og selv vælge om han vil prøve igen.
    #
    # HALEN, ikke præfikset: blokken skifter for hver fejl. Lagt i det stabile
    # præfiks brød den byte-identiteten mellem warmer og live-assembly —
    # TestCachePrefixInvariant fangede det med det samme, hvilket er præcis den
    # fejl der kostede ~91 % af det cacheable prefix i juni.
    try:
        from core.services.autonomous_run_failures import prompt_section as _arf_section
        _arf_text = _arf_section()
        if _arf_text:
            _dyn_tail.append(_arf_text)
            derived_inputs.append("fejlede autonome kørsler (user-msg tail)")
    except Exception:
        pass
...
# ── Private-lag-sektioner: udskilt til prompt_sections/private_layer_sections.py
# (Boy Scout, 2026-09-02). Re-eksporteret her så eksisterende kaldesteder og
# tests ikke brækker; ryddes op når de er flyttet naturligt.
from core.services.prompt_sections.private_layer_sections import (  # noqa: E402
    _visible_chronicle_context_section,
    _visible_current_pull_section,
    _visible_dream_residue_section,
    _visible_response_style_hint_section,
