...
def build_cognitive_frame(
    *,
    ...
    if perception.get("active"):
        perception_summary = str(perception.get("summary") or "")[:_MAX_SLICE_CHARS]
        if perception_summary:
            ...
            if salient and salient[0].get("source") in {"cognitive-episode", "theory-of-mind"}:
                salient = [salient[0], perception_item, *salient[1:]][:_MAX_SALIENT_ITEMS]
            else:
...
def build_cognitive_frame_prompt_section() -> str | None:
    ...
    perception = frame.get("perception_carry") or {}
    ...
    if perception.get("active"):
        directive = str(perception.get("directive") or "").strip()
        if directive:
...
def _safe_perception_surface() -> dict[str, object]:
    try:
        from core.services.perceptual_event_engine import build_perception_surface
