...
def list_all_surfaces() -> dict[str, Any]:
    ...
    try:
        ...
        observe_hub("signal_surface_router", meta={"surfaces": len(result), "errors": errs})
    except Exception:
        pass
    return result


def build_signal_surface_router_surface() -> dict[str, object]:
    ...
    return {
        "active": True,
        "mode": "signal-surface-router",
        "summary": "Module loaded; entry points available.",
        "authority": "derived-read-only",
    }


def _emit_signal_surface_router_event(kind: str, payload: dict[str, object] | None = None) -> None:
    """Emit a signal_surface_router-scoped event. Defensive — never blocks caller.
