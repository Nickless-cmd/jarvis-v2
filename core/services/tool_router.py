...
def _persist(
    ...
    try:
        with connect() as c:
            # HER SKAL VAGTEN IND — MEN FOERST NAAR HAENDELSEN UDGIVES.
            #
            # `projection_tool_router.guard_direct_write` findes og er testet,
            # og den var koblet ind her i et par timer den 12/9-2026. Den blev
            # koblet FRA igen, fordi vagten fjerner den eneste skriver:
            #
            #   * `KIND = "tool_router_decision"` optraeder ÉT sted i repoet —
            #     sin egen definition. Ingen udgiver haendelsen.
            #   * `session_handle.py:240-246` registrerer kun `chat_messages`,
            #     ved navn. Router-projektionen folder aldrig i produktion.
            #   * `chat-e58f16c561a6474` ER allerede i `ledger` og havde 82
            #     raekker her, den nyeste skrevet i det sekund vi maalte.
            #
            # Med vagten inde ville den session miste sin router-telemetri ved
            # naeste genstart — i tavshed, fordi afvisningen fanges nedenfor.
            # Vagtens egen fejlbesked siger «skriv haendelsen til hovedbogen i
            # stedet», og den vej findes ikke: `append_unowned` AFVISER en
            # ledger-session, saa en udgiver kraever et SessionHandle med lease
            # midt i en tur der maaske selv holder den.
            #
            # Raekkefoelgen er altsaa: udgiv haendelsen, generalisér
            # registreringsstedet, og KOBL SAA vagten ind. En vagt der spaerrer
            # den gamle vej foer den nye findes, er ikke et vaern — det er
            # tavst datatab.
            c.execute(
