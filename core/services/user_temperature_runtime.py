...
def build_user_temperature_runtime_surface() -> dict[str, object]:
    ...
    return {
        "active": True,
        "mode": "user_temperature_runtime",
        "summary": "Module loaded; entry points available.",
        "authority": "derived-read-only",
    }


def _emit_user_temperature_runtime_event(kind: str, payload: dict[str, object] | None = None) -> None:
    ...
    try:
        ...
        event_bus.publish(
            f"user_temperature_runtime.{kind}",
            payload or {},
        )
    except Exception:
        pass
