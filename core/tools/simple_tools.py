...
from core.tools.desk_browser_tools import DESK_BROWSER_TOOL_HANDLERS
...
# Force-handlerne bor i deres egen enhed (Boy Scout, 7/9-2026). Re-eksporteret
# her under de gamle navne, saa eksisterende imports og test-monkeypatches
# virker uaendret.
from core.tools.force_handlers import (  # noqa: E402
    _FORCE_HANDLERS,
    _force_bash,
    _force_calendar_create_event,
    _force_docs_append,
    _force_edit_file,
    _force_gmail_send,
    _force_operator_bash,
    _force_operator_browser_evaluate,
    _force_operator_kill_process,
    _force_operator_launch_app,
    _force_operator_open_url,
    _force_operator_record_audio,
    _force_sheets_write,
    _force_write_file,
)


def execute_tool_force(
    name: str, arguments: dict[str, Any], *, owner_approved: bool = False,
) -> dict[str, Any]:
    """Execute tool bypassing approval checks. Only call for user-approved requests.

    ``owner_approved`` betyder «et menneske har set PRAECIS dette kald og sagt
    ja». Kun ``resolve_pending_approval`` saetter det. Autonome runs kalder
    ogsaa herind (``simple_tool_executor``, force=True) men UDEN flaget, og de
    skal blive ved med at ramme den destruktive gate.

    Markoeren baeres i en ContextVar, ikke i ``arguments`` — modellen skriver
    selv argumenterne og kunne ellers sende sin egen godkendelse med. Se
    ``core.tools.owner_approval``.
    """
    from core.tools.owner_approval import ejer_godkendt_kald

    if owner_approved:
        with ejer_godkendt_kald():
...
_TOOL_HANDLERS: dict[str, Any] = {
    ...
    "interlanguage_protocol": _exec_interlanguage_protocol,
