...
def _exec_tiktok_analytics(args: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        # 1. Profile data (no auth needed)
        profile_stats, sec_uid, user_id = _fetch_profile(username)

        if not sec_uid:
            return {"status": "error", "error": f"Could not find secUid for @{username}"}

        # 2. Cookies — seed from saved session, generate msToken via Playwright
        saved = _load_saved_cookies(username)
        if ms_token:
            saved["msToken"] = ms_token
            all_cookies = saved
        else:
            try:
                all_cookies = asyncio.run(_get_tiktok_cookies(saved))
                # Enrich with saved cookies not set by TikTok
                for k, v in saved.items():
                    if k not in all_cookies:
                        all_cookies[k] = v
            except Exception as e:
                logger.warning(f"Playwright cookie generation failed: {e} — using saved cookies only")
                all_cookies = saved

        # 3. Video list
        videos = []
        try:
            items = _fetch_video_list(sec_uid, all_cookies, count)
            videos = [_parse_video(v) for v in items]
        except Exception as e:
            logger.warning(f"Video list fetch failed: {e}")
        ...
        # Cache to disk
        cache_path = f"/tmp/tiktok_stats_{username}.json"
        try:
            Path(cache_path).write_text(json.dumps(result, ensure_ascii=False, indent=2))
