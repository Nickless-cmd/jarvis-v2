...
status: forældet
audited: 2026-07-08
ground_truth: pyproject.toml specifies requires-python=3.11+ (not 3.10); no requirements.txt exists (ls verified); .pre-commit-config.yaml has 5 real hooks (detect-secrets, enforce-test-coverage, enforce-commit-hygiene, block-jvs-keys, startup-smoke) not documented; git log shows 2583+ commits
superseded_by: docs/CLAUDE.md
---
