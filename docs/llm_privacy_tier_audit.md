...
| `core/services/surprise_daemon.py` | `_generate_surprise` | baseline vs current internal state | 🔒 LOCAL-REQUIRED | `"Noget uventet skete med din tilstand"` |
| `core/services/conflict_daemon.py` | `_generate_conflict_phrase` | energy/mode/surprise conflict | 🔒 LOCAL-REQUIRED | indre konflikt og spænding |
| `core/services/curiosity_daemon.py` | `_generate_curiosity_signal` | thought fragments | 🔒 LOCAL-REQUIRED | `"Denne tanke fangede din opmærksomhed"` |
...
Kun disse bør flyttes til OllamaFreeAPI i første omgang:
...
- `surprise_daemon` — bruger private divergences i intern state.
...
- `experienced_time` / `absence` / `conflict` / `curiosity`-lignende daemons — alle er bundet til intern, personlig runtime-tilstand.
