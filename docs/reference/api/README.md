...
Generated per-package reference for `core/`+`apps/`+`scripts/`. 15016 functions/methods, 51% with docstrings. Undocumented public functions: see [`DOCSTRING_COVERAGE.md`](../DOCSTRING_COVERAGE.md).
...
- [`core.services.19`](core.services.19.md) — `prompt_section_reevaluation` … `relation_continuity_signal_tracking`
...
- [`core.services.22`](core.services.22.md) — `self_review_cadence_signal_tracking` … `signal_decay_daemon`
- [`core.services.23`](core.services.23.md) — `signal_delta_trigger` … `sustained_attention`
- [`core.services.24`](core.services.24.md) — `system_cartographer` … `tool_result_store`
- [`core.services.25`](core.services.25.md) — `tool_round_label` … `visible_model`
- [`core.services.26`](core.services.26.md) — `visible_model_adapters` … `witness_signal_tracking`
- [`core.services.27`](core.services.27.md) — `workspace_crypto` … `world_model_signal_tracking`
...
- [`scripts.02`](scripts.02.md) — `jarvis_bare_practice_runner` … `user_md_learned_migration`
- [`scripts.03`](scripts.03.md) — `validate_commit_attribution` … `verify_fase_a`
