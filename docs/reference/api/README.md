...
Generated per-package reference for `core/`+`apps/`+`scripts/`. 14752 functions/methods, 51% with docstrings. Undocumented public functions: see [`DOCSTRING_COVERAGE.md`](../DOCSTRING_COVERAGE.md).
...
- [`apps.api.jarvis_api.routes.02`](apps.api.jarvis_api.routes.02.md) — `interlanguage_blind` … `sensory`
- [`apps.api.jarvis_api.routes.03`](apps.api.jarvis_api.routes.03.md) — `status` … `workbench`
...
- [`core.services.25`](core.services.25.md) — `turn_trace` … `visible_runs`
- [`core.services.26`](core.services.26.md) — `visible_runs_approvals` … `world_model_signal_tracking`
...
- [`scripts.01`](scripts.01.md) — `__init__` … `laering_status`
- [`scripts.02`](scripts.02.md) — `ledger_rehearsal` … `verify_fase_a`
