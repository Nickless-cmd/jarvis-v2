"""«Ingen kreative projekter endnu» betød IKKE død.

`cognitive_architecture_surface.py` læser `active` som «systemet lever».
creative_projects satte nøglen efter om der var INDHOLD, så et modul der tikkede
...
`create_project` HAR en kalder (`seed_system`) — der var bare ikke sået et
projekt endnu."""
from __future__ import annotations

import core.services.creative_projects as M


def test_et_tomt_men_koerende_modul_er_LEVENDE():
    flade = M.build_creative_projects_surface()
...
def test_active_afhaenger_IKKE_af_en_taelling():
    ...
    kilde = inspect.getsource(M.build_creative_projects_surface)
