"""Personligt projekt: temaerne må kun komme fra ÉN bruger.
...
import core.services.personal_project as M
...
def optager(tmp_path, monkeypatch):
    ...
    c.executemany(
        ...
        "VALUES (?,?,?,?,?)",
        [("s1", "user", "bjoerns egen saetning her om noget", "2026-09-26T10:00:00+00:00", "bjorn"),
         ("s2", "user", "michelles private saetning om noget", "2026-09-26T10:01:00+00:00", "michelle")])
    c.commit()
    ...
    monkeypatch.setattr(M, "connect", lambda *a, **k: _C(), raising=False)
    yield opt
    c.close()
...
def _kald():
    try:
        M.detect_nomination_candidates(lookback_days=3650)
    except Exception:
