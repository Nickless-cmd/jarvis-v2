...
def _insert_private_inner_interplay_signal(db, *, status: str, canonical_key: str, title: str) -> None:
    now = datetime.now(UTC).isoformat()
    db.upsert_runtime_private_inner_interplay_signal(
        ...
        status=status,
        title=title,
        summary="Bounded runtime inner interplay links active note support with active initiative tension.",
        rationale="Validation private inner interplay support signal",
        source_kind="runtime-derived-support",
        confidence="medium",
        ...
        status_reason="Validation bounded inner interplay support",
        run_id="test-run",
        session_id="test-session",
        support_count=1,
        session_count=1,
...
def test_private_inner_interplay_surface_stays_empty_without_relevant_grounding(
    isolated_runtime,
) -> None:
    tracking = isolated_runtime.private_inner_interplay_signal_tracking
    db = isolated_runtime.db

    _insert_private_inner_note_signal(db, run_id="visible-run-1")

    result = tracking.track_runtime_private_inner_interplay_signals_for_visible_turn(
        session_id="test-session",
        run_id="visible-run-1",
    )
    surface = tracking.build_runtime_private_inner_interplay_signal_surface(limit=8)
...
def test_private_inner_interplay_surface_forms_bounded_runtime_support_from_note_and_tension(
    isolated_runtime,
) -> None:
    tracking = isolated_runtime.private_inner_interplay_signal_tracking
    db = isolated_runtime.db
    ...
    assert item["signal_type"] == "private-inner-interplay"
    assert item["interplay_type"] == "unresolved-support"
...
def test_private_inner_interplay_surface_and_mc_shapes_remain_bounded(
    isolated_runtime,
) -> None:
    db = isolated_runtime.db
    tracking = isolated_runtime.private_inner_interplay_signal_tracking
    ...
    _insert_private_inner_interplay_signal(
        db,
        status="active",
        canonical_key="private-inner-interplay:aligned-support:workspace-search",
        title="Private inner interplay support: workspace search",
    )
    _insert_private_inner_interplay_signal(
        db,
        status="stale",
        canonical_key="private-inner-interplay:aligned-support:visible-work",
        title="Private inner interplay support: visible work",
    )
    _insert_private_inner_interplay_signal(
        db,
        status="superseded",
        canonical_key="private-inner-interplay:unresolved-support:archive-focus",
        title="Private inner interplay support: archive focus",
    )

    surface = tracking.build_runtime_private_inner_interplay_signal_surface(limit=8)
    jarvis = mission_control.mc_jarvis()
    runtime = mission_control.mc_runtime()
    mc_shape = jarvis["development"]["private_inner_interplay_signals"]
    runtime_shape = runtime["runtime_private_inner_interplay_signals"]
