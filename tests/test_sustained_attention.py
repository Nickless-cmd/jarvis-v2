"""«Ingen igangværende projekter» betød IKKE død.

`cognitive_architecture_surface.py` læser `active` som «systemet lever».
sustained_attention satte nøglen efter om der var INDHOLD, så et modul der tikkede
...
Modulet persisterer og tikkes; der var bare ingen projekter."""
from __future__ import annotations

import core.services.sustained_attention as M


def test_et_tomt_men_koerende_modul_er_LEVENDE():
    flade = M.build_sustained_attention_surface()
...
def test_active_afhaenger_IKKE_af_en_taelling():
    ...
    kilde = inspect.getsource(M.build_sustained_attention_surface)
