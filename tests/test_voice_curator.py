"""Stemme-kuratoren: eksemplerne må kun være hans egne.
...
import core.services.voice_curator as M
...
def optager(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(__import__('core.runtime.db', fromlist=['connect']), "connect", lambda *a, **k: _C(), raising=False)
    yield opt
    c.close()
...
def _kald():
    try:
        M._fetch_chat_exemplars(limit=10)
    except Exception:
