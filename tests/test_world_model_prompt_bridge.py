...
def _self_report_block(visible_model, message: str) -> str:
    """Return the full, untrimmed RUNTIME SELF-REPORT GROUNDING block.

    The block is generated by ``_runtime_self_report_instruction`` and then
    fed through the attention-budget layer in ``_build_visible_input``, which
    trims the visible system prompt's self_report section to ~600 chars for
    token economy. That trim drops the fact/guard lines these tests assert on.
    To verify the *content generator* (its intent) we call it directly with
    the same readiness context the production path passes in, mirroring
    ``visible_model``'s ``runtime_self_report_context`` wiring.
    """
    from core.services import prompt_contract

    block = prompt_contract._runtime_self_report_instruction(
        user_message=message,
        runtime_self_report_context={
            "visible_execution_readiness": visible_model.visible_execution_readiness(),
        },
    )
    assert block is not None, "self-report block should be generated for this query"
    return block
...
def test_visible_support_blocks_remain_small_subordinate_helper_sections(
    ...
    # The five support sections are generated in full, then joined and fed
    # through the attention-budget layer in _build_visible_input, which trims
    # the visible system prompt's support_signals section to ~400 chars — so
    # only ~2 of the 5 blocks survive in the assembled prompt. To verify each
    # generated block is a small subordinate helper section (this test's
    # intent), call the section generator directly, before that budget trim.
    from core.services import prompt_contract

    support_blocks = [
        block
        for block in prompt_contract._visible_support_signal_sections(
            compact=False, include=True, user_message="Hello",
...
def test_visible_input_includes_runtime_self_report_grounding_for_backend_status_query(
    ...
    block = _self_report_block(
        isolated_runtime.visible_model,
        "hvad er din backend status?",
    )

    assert "RUNTIME SELF-REPORT GROUNDING" in block
    assert f"{_entity_name()}-specific" in block
    assert "NOT a generic OpenAI" in block
    assert "backend_provider=ollama" in block
    assert "backend_model=qwen3.5:9b" in block
    assert "backend_status=ready" in block
    assert "runtime_awareness_state=constrained" in block
...
def test_visible_input_includes_runtime_self_report_grounding_for_open_loop_query(
    ...
    assert "RUNTIME SELF-REPORT GROUNDING" in block
    assert "open_loop_count=1" in block
    assert "open_loop_state=open" in block
    assert "open_loop_current=Open loop: Visible work" in block
...
def test_visible_self_report_includes_self_action_limits_guard(
    ...
    assert "RUNTIME SELF-REPORT GROUNDING" in block
    assert "IMPORTANT SELF-ACTION LIMITS" in block
    assert (
    ...
    assert (
        "trying again" in block.lower() or "reconnecting" in block.lower()
    )
    assert "State observed runtime status only" in block
